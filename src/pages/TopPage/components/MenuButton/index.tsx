...
const MenuButton = (props: IProps): JSX.Element => {
  const renderIcon = () => {
    switch (props.label) {
      case 'Snack':
        return (
  ...
  return (
    <div className="relative hexagon hover:cursor-pointer  bg-gradient-to-r from-primary-300 to-primary-400 max-w-[116px] h-[134px]">
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col w-fit items-center">
        {renderIcon()}
        <span className="font-inter text-white text-semilarge">{props.label}</span>
      </div>
    </div>
  );
};
